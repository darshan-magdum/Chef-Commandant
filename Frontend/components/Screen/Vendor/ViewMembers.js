...
export default function ViewMembers({ navigation }) {
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`http://192.168.0.114:3000/api/vendormember/${selectedVendor._id}`);
  ...
  const handleConfirmEdit = async () => {
    try {
      ...
      await axios.put(`http://192.168.0.114:3000/api/vendormember/${selectedVendor._id}`, updatedVendor);
