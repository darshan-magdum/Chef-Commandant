...
export default function ViewMembers({ navigation }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const [vendorResponse, branchResponse] = await Promise.all([
        axios.get(`http://localhost:3000/api/vendormember/byvendor/${vendorId}`),
        axios.get(`http://localhost:3000/api/vendor/vendor/${vendorId}`)
      ]);
      console.log('Vendor Data:', vendorResponse.data); // Log vendor data
      setVendors(vendorResponse.data);
      setFilteredVendors(vendorResponse.data);
      const locations = branchResponse.data.locations;
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:3000/api/vendormember/${selectedVendor._id}`);
  ...
  const handleConfirmEdit = async () => {
    try {
      ...
      await axios.put(`http://localhost:3000/api/vendormember/${selectedVendor._id}`, updatedVendor);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Vendor Members</Text>
      </View>
  ...
  placeholder="Search By Vendor Member Name..."
