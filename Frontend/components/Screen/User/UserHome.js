...
export default function UserHome() {
  ...
  const fetchFoodItems = useCallback(async () => {
    try {
      const response = await axios.get('http://localhost:3000/api/vendorMemberFoodRoutes/getallfooditems');
      ...
      const updatedFoodItems = response.data.map(item => ({
        ...
        image: `http://localhost:3000/${item.foodImage ? item.foodImage.replace(/\\/g, '/') : ''}`, // Construct full URL if image is present
