...
export default function Orderview({ navigation }) {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      const response = await axios.get(`http://192.168.0.114:3000/api/UserOrdersRoutes/${userId}`);
  ...
  const handleConfirmEdit = async () => {
    try {
      ...
      const response = await axios.put(`http://192.168.0.114:3000/api/UserOrdersRoutes/edit/${selectedOrder._id}`, editedOrder);
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await axios.delete(`http://192.168.0.114:3000/api/UserOrdersRoutes/delete/${selectedOrder._id}`);
