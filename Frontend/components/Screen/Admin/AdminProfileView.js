...
export default function AdminProfileView({ navigation }) {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:3000/api/admin/${adminId}`);  // Fetch user details using user ID
...
{userData ? 
   ...
   <View style={styles.detailItem}>
     ...
     <Text style={styles.detailText}>{userData.companyName}</Text>
   </View>
   ...
   <View style={styles.detailItem}>
     ...
     <Text style={styles.detailText}>{userData.contactNo}</Text>
   </View>
 </View>
