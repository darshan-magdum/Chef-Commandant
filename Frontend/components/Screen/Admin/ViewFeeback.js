...
export default function ViewFeedback({ navigation }) {
  ...
  const fetchFeedback = async () => {
    try {
      const response = await axios.get('http://192.168.0.112:3000/api/user/feedback/get-all-feedback');
...
>
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`http://192.168.0.112:3000/api/user/feedback/delete-feedback/${deleteFeedbackId}`);
