...
export default function ViewVendors({ navigation }) {
  ...
  const fetchData = async () => {
    try {
      const [vendorResponse, branchResponse] = await Promise.all([
        axios.get('http://192.168.0.112:3000/api/vendor/getallvendor'),
        axios.get('http://192.168.0.112:3000/api/branchRoutes/viewallbranch')
      ]);
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`http://192.168.0.112:3000/api/vendor/${selectedVendor._id}`);
  ...
  const handleConfirmEdit = async () => {
    try {
      ...
      await axios.put(`http://192.168.0.112:3000/api/vendor/${selectedVendor._id}`, updatedVendor);
