...
export default function ViewVendors({ navigation }) {
  ...
  const fetchData = async () => {
    try {
      const [vendorResponse, branchResponse] = await Promise.all([
        axios.get('http://localhost:3000/api/vendor/getallvendor'),
        axios.get('http://localhost:3000/api/branchRoutes/viewallbranch')
      ]);
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:3000/api/vendor/${selectedVendor._id}`);
  ...
  const handleConfirmEdit = async () => {
    try {
      ...
      await axios.put(`http://localhost:3000/api/vendor/${selectedVendor._id}`, updatedVendor);
