...
export default ViewBranches = ({ navigation }) => {
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:3000/api/branchRoutes/deletebranch/${selectedBranch._id}`);
  ...
  const handleConfirmEdit = async () => {
    ...
    if (valid) {
      try {
        ...
        const response = await axios.put(`http://localhost:3000/api/branchRoutes/editbranch/${selectedBranch._id}`, updatedBranchData);
