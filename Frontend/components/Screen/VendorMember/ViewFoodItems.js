...
export default function ViewFoodItems({ navigation }) {
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await axios.delete(`http://192.168.0.107:3000/api/vendorMemberFoodRoutes/deletefooditem/${selectedFoodItem._id}`);
  ...
  <ScrollView style={styles.cardContainer}>
    <View key={filteredFoodItems[currentPage - 1]._id} style={styles.card}>
      {filteredFoodItems[currentPage - 1].foodImage ? (
        <Image
          source={{ uri: `http://192.168.0.107:3000/${filteredFoodItems[currentPage - 1].foodImage}` }}
